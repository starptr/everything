...
// Radarr (LinuxServer.io): the MOVIE counterpart of sonarr.libsonnet. Monitors/grabs movies, hands
// torrents to qbittorrent, then imports completed downloads by hardlinking them out of the
// qbittorrent save dir into a library tree. Radarr is Sonarr's movie sibling (same LinuxServer.io
// servarr base), so this lib is a mirror of sonarr.libsonnet with movie-specific values: WebUI/API
// port 7878 (Sonarr's is 8989) and the RADARR__ servarr env prefix (Sonarr's is SONARR__).
//
// No VPN sidecar (Radarr is not a torrent client -- it talks to qbittorrent's WebUI, the thing
// behind gluetun) and no config seed (Radarr writes config.xml itself on first boot, so there's
// nothing to seed -- contrast qbittorrent, which seeds qBittorrent.conf only-if-empty).
//
// Storage: /config holds a SQLite DB + config.xml that Radarr rewrites at runtime. SQLite over NFS
...
// (<mediaMountPath>/library/Animation Movies (Seadexarr), set as Radarr's root folder via buildarr)
// are one filesystem -- hardlinks and atomic moves require that.
//
// Config that we pin declaratively via servarr env overrides (RADARR__<SECTION>__<KEY>, double
// underscore; they win over config.xml on every boot): the API key (from a Secret, so it's stable
// and Prowlarr's link to Radarr is reproducible -- not the random key Radarr would otherwise mint
// on first boot), the explicit port, and the update settings that keep Radarr from EVER updating
// itself (mechanism=Docker -> the in-app updater is disabled; updates happen by rolling the image).
{
  new(
    ...
    apiKey,                             // required -> Radarr API key (from sops; surfaced via the Secret below)
    name='radarr',
    namespace='default',
    image=images.radarr.fullyQualifiedImageReferencePinned,
    port=7878,                          // Radarr's WebUI/API port
    ...
    // sonarr/openclaw stringData idiom; the env var below reads it via secretKeyRef.
    secret: {
      apiVersion: 'v1',
      kind: 'Secret',
            ...
            containers: [
              {
                ...
                env: [
                  ...
                  // Servarr config overrides (RADARR__<SECTION>__<KEY>). The RADARR__ prefix is the
                  // application's, independent of the `name` param; these override config.xml each boot.
                  { name: 'RADARR__SERVER__PORT', value: std.toString(port) },    // explicit; same source as containerPort/Service
                  { name: 'RADARR__UPDATE__MECHANISM', value: 'Docker' },         // container-managed -> disables Radarr's in-app updater
                  { name: 'RADARR__UPDATE__AUTOMATICALLY', value: 'false' },      // never auto-apply updates
                ...
                // /ping is Radarr's unauthenticated health endpoint (returns 200 regardless of the
    ...
    // Tailnet-only L7 ingress (no funnel), mirroring sonarr/qbittorrent/openclaw.
    ingress: {
