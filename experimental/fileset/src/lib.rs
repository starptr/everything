//! `fileset` — parse a gitignore-like manifest that decides which files to include,
//! and copy the included files out of a directory tree.
//!
//! A fileset manifest is one glob per line, compiled by `globset` with
//! `literal_separator(true)` and anchored gitignore-style: a pattern with a `/` in it
//! is anchored to the tree root, a pattern without one matches at any depth, and a
//! trailing `/` matches a directory's whole subtree. A leading `!` re-includes;
//! `@import <path>` inlines another manifest (path relative to the importing file).
//! Paths are matched tree-root-relative, later rules win, and an unmatched path
//! defaults to "included". See the README for the source-of-truth format docs.
...
impl Fileset {
    ...
    /// Whether a tree-root-relative path is included. Default include; scan rules in
    /// order and let the last match win (exclude drops, `!` re-includes).
...
/// Copy every file under `src` that `fileset` includes into `out`, preserving relative
/// structure. Skips symlinks and prunes `.git`/`.jj` before descending into them.
/// Returns the number of files copied.
pub fn filter_tree(src: &Path, out: &Path, fileset: &Fileset) -> Result<usize> {
    let src_root = src
        .canonicalize()
        .with_context(|| format!("resolving source dir {}", src.display()))?;
